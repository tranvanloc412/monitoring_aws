...
class ResourceScanner:
    ...
    def get_managed_resources(self, env: str) -> List[Resource]:
        ...
        if not self._managed_resources:
            self._managed_resources = self.get_resources_by_tag(tags, self.RESOURCE_CONFIG)
        return self._managed_resources

    def get_resources_by_tag(self, tags: Dict[str, str], resource_config: Dict[str, Dict[str, str]]) -> List[Resource]:
        if not tags:
        ...
        for resource_type, config in resource_config.items():
            ...
            for item in resources:
                resource = Resource(
                    ...
                    resource_name=next((tag["Value"] for tag in item.get("Tags", []) 
    ...
    def _fetch_resources_from_aws(self, tags: Dict[str, str], resource_type: str) -> List[Dict]:
        try:
