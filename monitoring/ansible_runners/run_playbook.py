...
def build_ansible_command(
    playbook_path: str, inventory_path: str, extra_vars: Optional[Dict[str, str]] = None
) -> list:
...
def run_ansible_playbook(
    ...
    logger.info(
        f"Executing Ansible playbook: {playbook_path} with inventory {inventory_path}"
    )
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            check=True,
            env=env_vars,  # capture_output=True for log store after
        )
